...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(4,
           varSelectInput("variable", 
                          "Select Variable to Display", 
                          data = Player %>% select("FG", "FGA", "FG%", "eFG%", "3P", "3PA", "3P%", "2P", "2PA", "2P%", "FT", "FTA", "FT%", "ORB", "DRB","TRB", "AST", "STL", "BLK", "TOV", "PF", "PTS", "FGMP"))
    )
  ),
...
```{r}
Shooting <- read_csv("Shooting data.csv")
```
...
```{r}
ggplot(Denver, aes(x= ORtg, y= DRtg, color = Tm)) +
  geom_point()
```
